...
- [[#copy-ssh-config-to-home-folder][Copy SSH Config To $HOME Folder]]
...
I prefer to place scripts in $HOME/.local/bin

* Clone The Repo

#+begin_src bash

    git clone https://github.com/loujackson902/git_sync.git $HOME/.local/bin

#+end_src

* Source Profile

Source the appropriate profile for your shell.
                    ...
                    [[ -f "$HOME/.bash-profile" ]]
                        then
                            source $HOME/.bash-profile
                                    ...
                                    [[ -f "$HOME/.zprofile" ]]
                                        then source $HOME/.zprofile
fi
#+end_src

* Copy SSH Config To $HOME Folder

When starting fresh I prefer to copy the contents of /etc/ssh/ssh_config to $HOME/.ssh/config.
So lets do that real quick.
...
* Generate SSH Keys
Make sure to generate some ssh keys to use with Git if you haven't already.
Then add the path to those keys to your .ssh/config file.
Don't forget to add them to Github/Gitlab.
I will name the key 'git' as an example.
Feel free to modify the name and path as needed for your system.

#+begin_src bash

    ssh-keygen -f $HOME/.ssh/git
